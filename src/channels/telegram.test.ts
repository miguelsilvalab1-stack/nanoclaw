...
const mockSynthesizeSpeech = vi.fn<(text: string) => Promise<Buffer | null>>();
vi.mock('../transcription.js', () => ({
  transcribeAudio: (buf: Buffer) => mockTranscribeAudio(buf),
  synthesizeSpeech: (text: string) => mockSynthesizeSpeech(text),
}));
...
function createVoiceCtx(overrides: {
  ...
  return {
    ...
    getFile: vi.fn().mockResolvedValue({
      file_path: overrides.filePath ?? 'voice/file_0.oga',
    }),
  };
}
...
describe('TelegramChannel', () => {
  ...
  describe('voice messages', () => {
    ...
    it('falls back to placeholder when file download throws', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const ctx = createVoiceCtx({});
      ctx.getFile.mockRejectedValue(new Error('Download failed'));
    ...
    it('sends TTS voice note after text reply', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      // Simulate a voice message arrival
      ...
      // Now send a reply — should trigger TTS
      mockSynthesizeSpeech.mockResolvedValue(Buffer.from('fake-audio'));
      await channel.sendMessage('tg:100200300', 'Hi there');

      expect(currentBot().api.sendMessage).toHaveBeenCalledWith(
        '100200300',
        'Hi there',
      );
      expect(mockSynthesizeSpeech).toHaveBeenCalledWith('Hi there');
      expect(currentBot().api.sendVoice).toHaveBeenCalledWith(
        '100200300',
        expect.objectContaining({ filename: 'response.ogg' }),
      );

      fetchSpy.mockRestore();
    });

    it('clears voice state after first reply (no TTS on second)', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // First reply — TTS sent
      mockSynthesizeSpeech.mockResolvedValue(Buffer.from('fake-audio'));
      await channel.sendMessage('tg:100200300', 'Reply 1');
      expect(currentBot().api.sendVoice).toHaveBeenCalledTimes(1);

      // Second reply — no TTS
      vi.clearAllMocks();
      await channel.sendMessage('tg:100200300', 'Reply 2');
    ...
    it('TTS failure is non-fatal (text still sent)', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // TTS will fail
      mockSynthesizeSpeech.mockRejectedValue(new Error('TTS broke'));
      ...
      // No voice sent (it threw)
      expect(currentBot().api.sendVoice).not.toHaveBeenCalled();

      fetchSpy.mockRestore();
    });

    it('does not send TTS if synthesizeSpeech returns null', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(currentBot().api.sendMessage).toHaveBeenCalled();
      expect(currentBot().api.sendVoice).not.toHaveBeenCalled();
