...
async function handleQrBrowser(
  ...
  try {
    const svg = execSync(
      `node --input-type=commonjs -e "const QR=require('qrcode');const data=process.env.QR_DATA;QR.toString(data,{type:'svg'},(e,s)=>{if(e)process.exit(1);process.stdout.write(s)})"`,
